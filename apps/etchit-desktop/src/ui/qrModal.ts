// Share-address modal. Renders an Autonomi address as a QR code, plus
// copyable text in both bare-hex and `autonomi://`-prefixed forms.
//
// Same layout used everywhere across the suite — fetch>it desktop /
// mobile and etch/it desktop / mobile — so a card shared from one app
// scans identically and looks visually consistent.

import { invoke } from "@tauri-apps/api/core";
import { save } from "@tauri-apps/plugin-dialog";

import { abbreviateAddress, renderExportCardSvg, renderQrSvg } from "../qr";
import { suppressNextClipboardImage } from "../screenshot/clipboardWatcher";
...
export function mountQrModal(host: HTMLElement): QrModalApi {
  ...
  const api: QrModalApi = {
    open(address, title) {
      ...
      // filename) but leave it editable so the sharer can override it
      // before exporting the card.
      titleInput.value = (title ?? "").trim();
      codeEl.textContent = abbreviateAddress(address);
      // Copper-coloured centre mark so the QR carries the brand chevron
      // even when the modal frame is cropped out of a screenshot.
  ...
  savePng.addEventListener("click", () => {
    ...
    void (async () => {
      ...
      try {
        const suggested = `etchit-${currentAddr.slice(0, 8)}.png`;
  ...
  copyPng.addEventListener("click", () => {
    ...
    void (async () => {
      ...
      try {
        // Self-write: the screenshot watcher would otherwise spot the
        // QR card we just put on the clipboard and offer to etch it.
        // Suppress one window so the next poll dedupes against our
        // own write instead of re-prompting.
        suppressNextClipboardImage();
